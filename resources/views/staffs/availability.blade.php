        ...
        <div class="box first" STYLE="margin-top:2%"><br/>
            ...
            <div class="tabbable"> <!-- Only required for left/right tabs -->
                <ul class="nav nav-tabs">
                    <li class="active"><a href="/availability" data-toggle="">Available.</a></li>
                    <li><a href="/reserve" data-toggle="">Reserved.</a></li>
                    ...
                    <div class="tab-pane active" id="tab1">
                        <div style="border:2px solid #c0c0c0; margin-left: 5%; width: 90%; border-radius: 10px 10px;">
                            ...
                            <table class="table table-bordered table-striped table-hover table-responsive" style="margin-left: 10%; width: 80%">
                                ...
                                @if(($roomNo->count() > 0))
                                    <?php $i=1?>
                                    @foreach($roomNo as $no)
                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>{{$no->room_no}}</td>
                                            <td>{{$no->roomCart()->first()->name}}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr><th colspan="8">No Room is Currently Available.</th></tr>
                                @endif
                                </tbody>
                            </table>
                        </div><br/><br/><br/><br/>
                ...
                <div class="tab-pane" id="tab3">
                    <div style="border:2px solid #c0c0c0; margin-left: 15%; width: 70%; border-radius: 10px 10px;">
                        <div class="center gap">
                            <h4>Checked In.</h4><hr style="margin-left: 20%; width:60%;"/>
                        </div>
                        <table class="table table-bordered table-striped table-hover table-responsive" style="margin-left: 10%; width: 80%">
                            <thead>
                            <tr>
                            ...
                            @if(($checkIn->count() > 0))
                                ...
                                @foreach($checkIn as $in)
                                    <tr>
                                        <td>{{$i++}}</td>
                                        <td>{{$in->roomNo()->first()->room_no}}</td>
                                        <td>{{$in->roomNo()->first()->roomCart()->first()->name}}</td>
                                        <td>{{$in->check_in->format('D, jS, M Y')}}</td>
                                        <td>{{$in->check_out->format('D, jS, M Y')}}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr><th colspan="8">No Room is Currently Available.</th></tr>
